...
- **`parcellate_scalar`** **replaces** **`parcellate_scalar_as_roi`**; bundles converted to PSEG (not DSEG) via `tractogram_to_pseg`, with threshold as a parameter
...
- `parcellate_time_series` — parcellate BOLD; outputs time series + coverage map
...
- **Taylor:** Update existing user stories — remove `parcellate_scalar_as_roi`, update `track_to_region` example to include FA/CBF scalar input
