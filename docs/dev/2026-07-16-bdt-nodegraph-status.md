...
- **NEW (2026-07-16): the `bdt` CLI runs.** `bdt/cli/run.py::main` is rewritten lean (xcp_d telemetry/`build_workflow`/reports cruft deleted) to parse args → `run_spec` → print written derivatives. Demonstrated end-to-end: `bdt <bids> <out> participant --datasets anat=… atlases=… --spec thickness_parc.yaml --participant-label 125511` writes the real `atlas-4S456Parcels_thickness.{pscalar.nii,tsv,json}`.
- **NEW (2026-07-17): BOLD/xcp_d cruft stripped (task #8/#15 done).** `cli/parser.py` trimmed (581→438 lines: dropped `--task-id`/`--dummy-scans`/`--ignore`/`--bids-filter-file`/carbon flags + the version-check banner); **~1763 lines of dead xcp_d modules deleted** (`cli/workflow.py` + the whole `workflows/` package + the stale `test/test_cli.py`); 5 now-unreferenced `config.py` fields removed. Verified: package/CLI/engine import clean, real CLI run still works, **80 pytest**, ruff-clean.
...
2. **`resample_subcortical` + `assemble_cifti`** — the whole-brain dense-CIFTI variant (32k cortex + MNI-2mm subcortex → den-91k). **Investigated 2026-07-17:** the **transform chain is ready** (fmriprep ships `from-T1w_to-MNI152NLin6Asym_xfm.h5`; compose with the computed ACPC↔T1w rigid → ACPC→MNI6, Strategy A `ApplyTransforms`), but there's an **atlas gap: the AtlasPack has NO subcortical-structure grid** (only whole-brain 4S parcellations; no `HCPSubcortical`/CIFTI subcortical-structure dseg defining the 31870 subcortical grayordinates that `assemble_cifti` needs). Unlike the cortex case there's no self-contained restrict workaround — the subcortical structure grid (e.g. HCP `Atlas_ROIs.2.nii.gz` / a templateflow MNI152NLin6Asym subcortical dseg) would need fetching. **Decision for the user:** which subcortical atlas + confirm the MNI target (MNI152NLin6Asym-2mm per the story).
