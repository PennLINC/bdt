...
.. warning::
   ...
   BDT currently only generates denoised data in MNI152NLin6Asym space.

#####################################################################
Defining standard and nonstandard spaces where data will be resampled
#####################################################################

The command line interface of *BDT* allows resampling the preprocessed data
...
.. important::
   *BDT* will reduce the amount of output spaces to just spaces listed in ``--output-spaces``,
...
that allows end-user applications such as *BDT* to flexibly query and pull
template and atlas information.
In other words, *TemplateFlow* enables *BDT* to access a wide range
of templates (and also custom templates, see below).
Therefore, *TemplateFlow* is central to define *BDT*'s interface regarding
...
When using *BDT* in a workflow that will investigate effects that span across
...
For instance, to instruct *BDT* to use the MNI template brain distributed with
FSL as coordinate reference the option will read as follows: ``--output-spaces MNI152NLin6Asym``.
By default, *BDT* uses ``MNI152NLin6Asym`` as spatial-standardization reference.
...
Therefore, *BDT* will run nonlinear registration processes against the template
T1w image corresponding to all the standard spaces supplied with the argument
``--output-spaces``.
By default, *BDT* will resample the preprocessed data on those spaces (labeling the
...
To make your custom templates visible by *BDT*, and usable via
...
* ``fsnative``: similarly to the ``anat`` space for volumetric references,
  including the ``fsnative`` space will instruct *BDT* to sample the
...
In other words, running *BDT* with ``--output-spaces MNI152NLin6Asym:res-2
