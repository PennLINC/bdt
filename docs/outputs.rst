...
*BDT* outputs conform to the :abbr:`BIDS (brain imaging data structure)`
Derivatives specification (see `BIDS Derivatives`_, along with the
upcoming `BEP 011`_ and `BEP 012`_).
*BDT* generates three broad classes of outcomes:
...
1.  **Visual QA (quality assessment) reports**:
    ...
    of processing and ensures the transparency of *BDT* operation.
...
Assuming BDT is invoked with::
    ...
    bdt <input_dir>/ <output_dir>/ participant [OPTIONS]
...
``dataset_description.json`` is a metadata file in which BDT
...
*BDT* outputs summary reports,
written to ``<output dir>/bdt/sub-<label>.html``.
...
For each :abbr:`BOLD (blood-oxygen level dependent)` run processed with *BDT*,
...
*BDT* outputs a set of confounds that can be used to denoise the data.
...
The confounds generated by *BDT* are ICA component time series
...
See implementation on :mod:`~bdt.workflows.bold.confounds.init_bold_confs_wf`.
