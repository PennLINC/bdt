# Maintenance instructions for BDT
...
2. **Bump the date tag** in `Dockerfile` to today's date:
   ...
   ```dockerfile
   ARG BASE_IMAGE=pennlinc/bdt-base:<YYYYMMDD>
   ```
...
4. **Verify** the CI image jobs succeed and the new base image
   appears on Docker Hub at `pennlinc/bdt-base:<YYYYMMDD>`.
...
2. **Clone and check out a branch:**
   ...
   ```bash
   git clone https://github.com/PennLINC/bdt.git
   cd bdt
...
- The main Docker image is built **FROM** `pennlinc/bdt-base:<YYYYMMDD>` (see `Dockerfile`). The tag is a date stamp (e.g. `20260219`) indicating when the base was last built.
...
- To release a new base image:
  1. Update the date tag in **Dockerfile** (`ARG BASE_IMAGE=pennlinc/bdt-base:YYYYMMDD`).
