...
def _combinations(resolved: dict[str, list[Match]]):
    """Cartesian product over multi-match selections -> one dict per combination."""
    names = list(resolved)
    for combo in itertools.product(*(resolved[name] for name in names)):
...
def run_spec(
    ...
    for subject in run_subjects:
        ...
        for combo_idx, combo in enumerate(_combinations(resolved_all)):
            sel_paths = {name: match.path for name, match in combo.items()}
            plan = build_sink_plan(spec, combo, roots)
