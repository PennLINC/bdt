...
``multiprocessing.Process`` that allows bdt to enforce
...
def build_workflow(config_file, retval):
    ...
    banner = [f'Running BDT version {version}']
    notice_path = Path(pkgrf('bdt', 'data/NOTICE'))
    if notice_path.exists():
        banner[0] += '\n'
        banner += [f'License NOTICE {"#" * 50}']
        banner += [f'BDT {version}']
    ...
    retval['workflow'] = init_bdt_wf()

    # Check workflow for missing commands
    missing = check_deps(retval['workflow'])
    if missing:
        build_log.critical(
            'Cannot run BDT. Missing dependencies:%s',
    ...
    build_log.info(
        'BDT workflow graph with %d nodes built successfully.',
...
def build_boilerplate(config_file, workflow):
    ...
    if not config.execution.md_only_boilerplate and citation_files['md'].exists():
        ...
        bib_text = Path(pkgrf('bdt', 'data/boilerplate.bib')).read_text()
        citation_files['bib'].write_text(
            bib_text.replace(
                'BDT <version>',
                f'BDT {config.environment.version}',
            )
        )
        ...
        cmd = [
            ...
            '--citeproc',
            '--metadata',
            'pagetitle="BDT citation boilerplate"',
