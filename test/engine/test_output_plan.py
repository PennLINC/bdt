...
def test_node_output_entities_injects_atlas_and_stat():
    ...
    assert fc['stat'] == 'pearsoncorrelation'
...
def test_cifti_input_plans_native_and_tsv():
    ...
    assert coverage.entities['stat'] == 'coverage'
    assert parc[0].entities['stat'] == 'mean'
...
def test_nifti_input_plans_tsv_only():
    ...
    # A volumetric input yields the TSV product only (no native CIFTI).
...
def test_parcellate_scalar_preserves_source_naming():
            ...
            'nodes': [
                {
                    ...
                    'action': 'select_data',
                    'dataset': 'xcpd',
                    'filters': {'suffix': 'boldmap', 'stat': 'alff'},
                },
                {
                    'name': 'atlas',
                    'action': 'select_atlases',
    ...
    resolved = {
        'load_alff': Match(
            ...
            entities={
                'sub': '01', 'space': 'fsLR', 'den': '91k', 'stat': 'alff',
    ...
    assert prods[0].entities['stat'] == 'alff'  # not overwritten with 'mean'
